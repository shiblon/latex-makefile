...
# Outputs all generated index files to STDOUT (discovered by reading *.fls
# OUTPUT lines and producing corresponding .ind filenames as needed).
#
# $(call get-generated-index-names,<source recorder file (*.fls)>)
define get-generated-index-names
[ -f '$1' ] && \
$(SED) \
	-e '/^OUTPUT .*\.idx$$/{' \
	-e '  s/^OUTPUT //p' \
...
# All currently-known index files, taken from the .fls files in this directory.
# NOTE: this is likely to be somewhat fragile, since it depends on a relatively
# successful build.  We may be able to just delete all .idx files, but I'm
# hesitant to do that, since sometimes those are generated by hand.
#
# TODO
# TODO
# TODO
...
# CLEAN TARGETS
#
# clean-index is somewhat unique - it relies on the .fls file being properly
# built so that it can determine which of the index files was generated, and
...
clean-index:
	$(QUIET)$(call clean-files,$(foreach e,$(addsuffix .fls,$(all_stems_source)),\
						$(shell $(call get-generated-index-names,$e))))
...
.PHONY: clean
clean:	clean-index clean-tex clean-graphics clean-deps clean-backups clean-auxiliary
