...
# already has them), and relies on bibliographies that may not exist.
#
...
.PHONY: clean-deps
clean-deps:
	$(QUIET)$(RM) $(all_d_targets) *.make *.make.temp *.cookie
...
# for doing it right.
clean-graphics:
	$(QUIET)$(RM) $(all_graphics_targets) *.gpi.d *.pstex{_t,}

.PHONY: clean-backups
clean-backups:
	$(QUIET)$(RM) $(backup_patterns)
